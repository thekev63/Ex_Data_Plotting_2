...
# Assuming this is the ON-ROAD type in NEI
NEI_OnRoad <- NEI[NEI$fips=="24510" & NEI$type=="ON-ROAD",  ]

aggregatedTotalByYear <- aggregate(Emissions ~ year, NEI_OnRoad, sum)
...
myplot <- myplot + geom_bar(stat="identity") +
  ...
  ggtitle('Total Emissions from (ON-ROAD) motor vehicle in Baltimore City - 1999 to 2008')
